...
///////////////////////////////////////////////////////////////////////////////
// Unlike C3D_Tex (3DS), GXTexObj is just a descriptor - it does not own the
// pixel storage backing it, so each piece tracks its own tiled texel buffer
// (allocated 32-byte aligned so it can be cache-flushed for the GX FIFO).
///////////////////////////////////////////////////////////////////////////////
...
enum PixelFormat
{
	...
	PixelFormat_A4R4G4B4			=			0x0002, // stored as GX_TF_RGB5A3, closest native GX format
...
public:
	...
	GXTexObj* GetTexture(int x, int y, int &width, int &height, float &u1, float &v1, float &u2, float &v2);
	GXTexObj* GetTextureF(float x, float y, float &width, float &height, float &u1, float &v1, float &u2, float &v2);
